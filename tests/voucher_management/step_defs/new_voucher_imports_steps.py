...
def make_pre_existing_vouchers(
    ...
    pre_existing_vouchers: List[Reward] = create_mock_vouchers(
...
def check_new_vouchers_imported(request_context: dict, carina_db_session: "Session") -> None:
    ...
    vouchers: Optional[List[Reward]] = None
    for i in range(7):
        logging.info("Sleeping for 10 seconds...")
        sleep(10)
        vouchers = (
            carina_db_session.execute(select(Reward).where(Reward.voucher_code.in_(new_codes + pre_existing_codes)))
            .scalars()
            .all()
        )
...
def check_vouchers_not_imported(carina_db_session: "Session", request_context: dict) -> None:
    vouchers = (
        carina_db_session.execute(
            select(Reward).where(Reward.voucher_code.in_(request_context["import_file_new_voucher_codes"]))
        )
        .scalars()
        .all()
    )
