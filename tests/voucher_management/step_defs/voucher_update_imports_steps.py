...
def _get_voucher_update_rows(
    ...
    voucher_update_rows = (
        carina_db_session.execute(
            ...
            .join(Reward)
            .where(
                Reward.voucher_code.in_(voucher_codes),
...
def _get_voucher_row(carina_db_session: "Session", voucher_code: str, req_date: str, deleted: bool) -> Reward:
    return (
        carina_db_session.execute(
            select(Reward).where(
                Reward.voucher_code == voucher_code,
...
def voucher_updates_upload(
    ...
    """
    # GIVEN
    mock_vouchers: List[Reward] = create_mock_vouchers(
